...
public class JChromaSDK {
    ...
    private JChromaSDK() {
        String libName = "/X86X64/CChromaEditorLibrary";
        String pa="";
        ...
        try {
			...
			InputStream isJarDll = JChromaLib.class.getResourceAsStream(libName);
			File tempDll = File.createTempFile("CChromaEditorLibrary.dll", "");
			...
			osJarDll.close();
			isJarDll.close();
                        pa=tempDll.getAbsolutePath();			
		}
		catch (IOException e) {
			System.out.println("Unable to extract the CChromaEditorLibrary.dll from the jar."); 
		}

        wrapper = (JChromaLib)Native.loadLibrary(pa, JChromaLib.class);
    }
	...
	*/
	...
	{
		return wrapper.PluginPreviewFrame(animationId, frameIndex);
	}
	/*
	D suffix for limited data types.
	*/
	/// EXPORT_API double PluginPreviewFrameD(double animationId, double frameIndex);
	public double previewFrameD(double animationId, double frameIndex)
	{
		return wrapper.PluginPreviewFrameD(animationId, frameIndex);
	}
	/*
	Displays the `Chroma` animation frame on `Chroma` hardware given the `frameIndex`. 
	...
	*/
	...
	public void previewFrameName(String path, int frameIndex)
	{
		wrapper.PluginPreviewFrameName(path, frameIndex);
	}
	/*
